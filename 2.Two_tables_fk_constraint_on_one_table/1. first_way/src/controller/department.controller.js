...
const deleteDepartment = async (req, res) => {
    try {
        ...
        return res.status(200).send({ message: count == 1 ? `Record deleted of department ${req.params.dept_id}` : `No record found for name ${req.params.dept_id}` });
    } catch (e) {
...
const updateDepartment = async (req, res) => {
    try {
        ...
        return res.status(200).send({ message: data[0] === 1 ? `Redord update for ${req.params.dept_name}` : `No record found for ${req.params.dept_name}` });
    } catch (e) {
        return res.status(400).json(e.message);
    }
};

module.exports = { registerDept, getAllDepartments, getDepartmentByName, deleteDepartment, updateDepartment };
